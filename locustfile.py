...
class CASTaskSet(TaskSet):
    ...
    def authenticate_to_service(self):
        ...
        if datetime.datetime.now() >= self.expiration:
            ...
            self.reset()
            return
        service = '''https://exampleService.uni.edu/cas/login/?origin=%2Femployees%3F'''
        client = self.client
    ...
    def employee_page(self):
        ...
        if datetime.datetime.now() >= self.expiration:
            ...
            self.reset()
            return
        service = '''https://exampleService.uni.edu/employees'''
        client = self.client
        print("Obtaining service ticket ...")
        with client.get("/cas/login", catch_response=True, params={'service': service}, allow_redirects=True) as response:
            if response.status_code == 200:
                response.success()
                # location = response.headers['Location']
            else:
                response.failure("Got status code {0} while loading employees ".format(response.status_code))
                return   
        response = client.get("https://exampleService.uni.edu/employees")
        response = client.get("https://exampleService.uni.edu/timereporting")
        response = client.get("https://exampleService.uni.edu/myaccountsummary")
    ...
    def students_page(self):
        ...
        if datetime.datetime.now() >= self.expiration:
            ...
            self.reset()
            return
        service = '''https://exampleService.uni.edu/students'''
        client = self.client
        print("Obtaining service ticket ...")
        with client.get("/cas/login", catch_response=True, params={'service': service}, allow_redirects=True) as response:
            if response.status_code == 200:
                ...
                response.failure("Got status code {0} while loading employees ".format(response.status_code))
                return   
        response = client.get("https://exampleService.uni.edu/students")
        response = client.get("https://exampleService.uni.edu/myaccountsummary")
        response = client.get("https://exampleService.uni.edu/academicprofile")
...
class CASLocust(HttpLocust):
    task_set = CASTaskSet
    host = 'https://cas_example.uni.edu'
    second = 1000
    min_wait = 5 * second
    max_wait = 15 * second
    creds = load_creds()
